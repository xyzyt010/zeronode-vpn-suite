...
/// Prefer `route print -4` over PowerShell — silent (CREATE_NO_WINDOW) and
/// never flashes a console during Outline/Tor connect/disconnect.
pub(crate) fn get_default_gateway_ip_by_cmd() -> std::io::Result<IpAddr> {
    ...
    for line in stdout.lines() {
        ...
        if cols.len() >= 3 && cols[0] == "0.0.0.0" && cols[1] == "0.0.0.0" {
            if let Ok(ip) = cols[2].parse::<IpAddr>() {
    ...
    Err(std::io::Error::other("No default gateway found via route print"))
}

#[allow(dead_code)]
pub(crate) fn get_default_gateway_interface() -> std::io::Result<String> {
    // Parse `route print -4` for the interface of the default route (column 4).
    let out = run_command("route", &["print", "-4"])?;
    let stdout = String::from_utf8_lossy(&out);
    for line in stdout.lines() {
        let cols: Vec<&str> = line.split_whitespace().collect();
        if cols.len() >= 4 && cols[0] == "0.0.0.0" && cols[1] == "0.0.0.0" {
            // cols[3] is interface IP on Windows route print, not alias —
            // return gateway IP as a stable fallback label.
            return Ok(cols[3].to_string());
        }
    }
    Err(std::io::Error::other("No default gateway interface found"))
}
